...
class Simulation:
    ...
    def simulate(self, df: DataFrame, df_eval: DataFrame,
                 symbol: str, factor_stop: float, factor_limit: float,
                 force: bool = False, is_hour = False) -> tuple[DataFrame, DataFrame]:
        suffix = "_hour" if is_hour else ""

        buy_path = f"simulation_buy{symbol}_16{factor_stop}{factor_limit}h10_v3{suffix}.csv"
        sell_path = f"simulation_sell{symbol}_16{factor_stop}{factor_limit}h10_v3{suffix}.csv"

        if not self._cache.simulation_exist(buy_path) or force:
            buy = self._simulate_fixed_timeframe(
                action="buy",
                df=df, df_eval=df_eval,
                stop_factor=factor_stop, limit_factor=factor_limit, is_hour=is_hour
            )
    ...
    def get_signals(self, symbol: str, df: DataFrame, indicators: Indicators, predictor_class, force: bool = False, is_hour =False) -> None:
        suffix = "_hour" if is_hour else ""
        for indicator in indicators.get_all_indicator_names():
            path = f"signal_{symbol}_{indicator}{suffix}.csv"
    ...
    def create_combined_indicator_data(self, indicators: Indicators, symbol: str, is_hour =False) -> DataFrame:
        df_list = []
        suffix = "_hour" if is_hour else ""
        for indicator in indicators.get_all_indicator_names():
            try:
    ...
    def _simulate_fixed_timeframe(self,
        ...
        for i, row in df.iterrows():
            ...
            if action == "buy":
                entry_price += spread  # Kauf zum Ask – du zahlst Spread sofort

            elif action == "sell":
                entry_price -= spread  # Verkauf zum Bid – aber Rückkauf zum Ask -> Spread wird sofort "verloren"
            ...
            for j, future_row in future.iterrows():
                ...
                if j > 0:
                    if is_hour:
                        accumulated_time += timedelta(minutes=5)
                    else:
                        accumulated_time += timedelta(hours=1)

                if action == "buy":
                    if current_low <= entry_price - stop_loss:
                        exit_time, exit_price, profit = current_time, entry_price - stop_loss, -stop_loss
                        break
                    elif current_high >= entry_price + take_profit:
                        exit_time, exit_price, profit = current_time, entry_price + take_profit, take_profit
                        break

                elif action == "sell":
                    if current_high >= entry_price + stop_loss:
                        exit_time, exit_price, profit = current_time, entry_price + stop_loss, -stop_loss
                        break
                    elif current_low <= entry_price - take_profit:
                        exit_time, exit_price, profit = current_time, entry_price - take_profit, take_profit
                        break
