...
class SRBreak(BasePredictor):
    ...
    def predict(self, df: DataFrame) -> str:
        ...
        for i in range(len(levels)):
            ...
            if current_close > l.upper:
                was_under = len(p1[p1.low < l.upper]) > 7
                up_trend = len(p1[p1.close > p1.EMA_20]) > 7

                if was_under and up_trend:
                    self._viewer.print_level(df[-7:-6].date.values[0], df[-1:].date.values[0], l.upper, l.lower, "Red")
                    return self.BUY

            if current_close < l.lower:
                was_over = len(p1[p1.high > l.lower]) > 7
                donw_trend = len(p1[p1.close < p1.EMA_20]) > 7

                if was_over and donw_trend:
                    self._viewer.print_level(df[-7:-6].date.values[0], df[-1:].date.values[0], l.upper, l.lower, "Red")
                    return self.SELL
    ...
    def get_training_sets(version:str):
        ...
        sr1 = SRBreak._sr_trainer(version)
        sr2 = SRBreak._sr_trainer2(version)
