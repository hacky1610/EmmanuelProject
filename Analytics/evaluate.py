...
for market in IG.get_markets_offline():
    if ps.count_of_all_by_symbol(market["symbol"]) == 0:
        continue

    df, df_eval = tiingo.load_test_data(market["symbol"], DataProcessor(), trade_type=TradeType.FX,
                                                            use_cache=True, days=30)

    for pred in ps.load_all_by_symbol(market["symbol"]):

        predictor_object = DeepPredictor(market["symbol"], cache, Indicators(), config=pred)
        predictor_object.setup(pred)
