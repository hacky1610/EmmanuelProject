...
class LogglyTracer(Tracer):
    ...
    def debug(self, message):
        self._log.create_input(f"[TI {self.type}] | DEBUG | {self._prefix} | {self._get_function()} | {message}")

    def error(self, message):
        self._log.create_input(f"[TI {self.type}] | ERROR | {self._prefix} | {self._get_function()} | {message}")
